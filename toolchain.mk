...
sources:$(srcdirs)
	for d in $(srcdirs); do cd "$$d" && git pull --depth=1; done

$(TOPDIR)/src/elf2flt:
	cd $(TOPDIR)/src && git clone --depth=1 -b riscv https://github.com/chmmn/elf2flt.git

$(TOPDIR)/src/linux:
	cd $(TOPDIR)/src && git clone --depth=1 -b riscv https://github.com/chmmn/linux.git

$(TOPDIR)/src/uclibc-ng:
	cd $(TOPDIR)/src && git clone --depth=1 -b riscv https://github.com/chmmn/uclibc-ng.git

$(TOPDIR)/src/riscv-binutils-gdb:
	cd $(TOPDIR)/src && git clone --depth=1 https://github.com/riscv/riscv-binutils-gdb.git

$(TOPDIR)/src/riscv-gcc:
	cd $(TOPDIR)/src && git clone --depth=1 https://github.com/riscv/riscv-gcc.git
...
binutils_build:
	cd $(TOPDIR)/build/binutils && make -j$(nproc)
binutils_install:
...
gcc1_build:
	cd $(TOPDIR)/build/gcc1 && make -j$(nproc)
gcc1_install:
...
gcc2_build:
	cd $(TOPDIR)/build/gcc2 && make -j$(nproc)
gcc2_install:
